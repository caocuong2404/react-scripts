...
inquirer
  ...
  .then(answer => {
    ...
    if (appPackage.devDependencies) {
      // We used to put rs-webpack in devDependencies
    ...
    if (fs.existsSync(paths.appTypeDeclarations)) {
      try {
        ...
        // Remove rs-webpack reference since they're getting a copy of the types in their project
        content =
          content
            // Remove rs-webpack types
            .replace(
              /^\s*\/\/\/\s*<reference\s+types.+?"rs-webpack".*\/>.*(?:\n|$)/gm,
              ''
            )
            .trim() + os.EOL;
    ...
    if (ownPath.indexOf(appPath) === 0) {
      try {
        // remove rs-webpack and rs-webpack binaries from app node_modules
        Object.keys(ownPackage.bin).forEach(binKey => {
    ...
    if (fs.existsSync(paths.yarnLockFile)) {
      ...
      if (process.platform === 'win32') {
        ...
        // Yarn is diligent about cleaning up after itself, but this causes the rs-webpack.cmd file
